...
@st.cache_data
def getData1():
    df = pd.read_csv('data/raw_updated_parsed_mutations.csv')
...
@st.cache_data
def getData2():
    df2 = pd.read_csv('data/cumulative_raw_updated_parsed_mutations.csv')
    df2['Week'] = pd.to_datetime(df2['Week'])
    df2['Week'] = [datetime.strptime(
        str(x), '%Y-%m-%d').strftime('%b-%d-%Y') for x in df2['Week'].dt.date]
    df2 = df2.iloc[1:, :]
    return df2
...
def getData3():
    ...
    df3 = df3.T
    df3 = df3.reset_index()
    df3 = df3.rename(columns={'index': 'Mutation'})
